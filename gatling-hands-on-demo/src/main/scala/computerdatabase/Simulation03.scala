	...
	def apply = {
		...
		val scn = scenario("Gatling simulation")
			.exec(http("Index page")
				.get("/"))
			.pause(11 milliseconds) // A 11ms pause
			.exec(http("Add computer page")
				.get("/computers/new"))
			.pause(2) // Identical to .pause(2 seconds)
			...
			.pause(1 second, 3 seconds) // A pause between 1s and 3s
			.exec(http("My computer page")
				.get("/computers/1000"))
			.pauseExp(200 milliseconds) // A pause with a mean of 200ms
			.exec(http("Index page")
				.get("/"))
